...
import { usePokemonDetail, EvolutionChain } from '../hooks/usePokemonData';
import { IoArrowBack, IoMale, IoFemale } from 'react-icons/io5';
...
const PokemonDetailModal: React.FC<PokemonDetailModalProps> = ({ pokemonName, onClose }) => {
  ...
  // Diagram Bundar untuk Capture Rate
  ...
  // Renderer untuk evolusi
  const EvolutionRenderer = () => <p>Evolution feature placeholder.</p>;

  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex justify-center items-center p-2">
      <div className="w-full max-w-sm h-[95vh] flex flex-col bg-white rounded-3xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className={`relative ${bg} pt-4 pb-4 flex-shrink-0 z-10 rounded-b-3xl`}>
          <button onClick={onClose} className="absolute top-4 left-4 text-white/80 hover:text-white">
            <IoArrowBack size={28} />
          </button>
          <div className="relative h-44 w-full">
             {detail?.sprites?.other['official-artwork']?.front_default && (
                <Image src={detail.sprites.other['official-artwork'].front_default} alt={detail.name} layout="fill" objectFit="contain" className="drop-shadow-2xl" priority />
             )}
          </div>
        </div>
        ...
        <div className="flex-grow overflow-y-auto -mt-12">
           <div className="pt-16 p-5 bg-white rounded-t-3xl">
              ...
              <div className="flex justify-center gap-4 my-6">
                <button onClick={() => setActiveTab('STATS')} className={`font-bold py-2 px-6 rounded-full ${activeTab === 'STATS' ? `${button} text-white` : 'bg-gray-200 text-gray-500'}`}>Stats</button>
                <button onClick={() => setActiveTab('EVOLUTIONS')} className={`font-bold py-2 px-6 rounded-full ${activeTab === 'EVOLUTIONS' ? `${button} text-white` : 'bg-gray-200 text-gray-500'}`}>Evolution</button>
              </div>
              ...
              {activeTab === 'EVOLUTIONS' && ( <EvolutionRenderer /> )}
            </div>
        </div>
      </div>
    </div>
